...
| Off-chain tx | **MeshJS** (`@meshsdk/core`) + **Blockfrost** | Wallet/tx plumbing for Preprod payments and loading the compiled Aiken blueprint. Blockfrost provides a live, read-only chain tip (honest evidence the system talks to Cardano); the quarantine gate is evaluated against the same compiled validators locally — mode `simulated`. Live transaction submission is the one remaining on-chain step (roadmap), intentionally gated off so nothing can claim a tx hit the chain when it didn't. |
